...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'invoicedOn' | 'dueOn'> & {
...
export class InvoiceService {
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...invoice,
      invoicedOn: invoice.invoicedOn?.toJSON() ?? null,
      dueOn: invoice.dueOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...restInvoice,
      invoicedOn: restInvoice.invoicedOn ? dayjs(restInvoice.invoicedOn) : undefined,
      dueOn: restInvoice.dueOn ? dayjs(restInvoice.dueOn) : undefined,
    };
  }
