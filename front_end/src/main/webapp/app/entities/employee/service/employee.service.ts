...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'startedOn' | 'terminatedOn'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      startedOn: employee.startedOn?.toJSON() ?? null,
      terminatedOn: employee.terminatedOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      startedOn: restEmployee.startedOn ? dayjs(restEmployee.startedOn) : undefined,
      terminatedOn: restEmployee.terminatedOn ? dayjs(restEmployee.terminatedOn) : undefined,
    };
  }
